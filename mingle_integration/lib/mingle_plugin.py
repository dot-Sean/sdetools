...
class MingleConnector(AlmConnector):
    ...
    def alm_get_task (self, task):
        ...
        if (card_element.length > 0):
            ...
            if (card_element.item(0).getElementsByTagName('modified_on')):
                modified_date =  card_element.item(0).getElementsByTagName(
						'modified_on').item(0).firstChild.nodeValue

            if (card_element.item(0).getElementsByTagName('property')):
    ...
    def alm_add_task(self, task):
        ...
        try:
            ...
            pass
        try:
            status_args = {'card[name]':task['title'],
                    'card[card_type_name]':
                            self.sde_plugin.config['mingle_card_type'],
                    'card[description]':
							self.sde_get_task_content(task),
                    'card[properties][][name]':
							'status',
					'card[properties][][value]':
							self.sde_plugin.config['mingle_new_status']}
            ...
            raise AlmException('Please check ALM-specific settings in config' 
					'file. Unable to add task %s because of %s' % (task['id'], 
					err))
        ...
        if (status=='DONE') or (status=='NA'):
            try:
                status_args = {'card[properties][][name]':'status',
                        'card[properties][][value]': self.sde_plugin.config[

...
class AlmPluginTestBase(object):
    ...
    @classmethod
    def initTest(self, PATH_TO_ALM_CONNECTOR):
        self.mock_1 = patch('%s.AlmConnector.sde_connect' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_sde_connect).start()
        self.mock_2 = patch('%s.AlmConnector.is_sde_connected' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_is_sde_connected).start()
        self.mock_3 = patch('%s.AlmConnector.sde_get_tasks' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_sde_get_tasks).start()
        self.mock_4 = patch('%s.AlmConnector.sde_get_task' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_sde_get_task).start()
        self.mock_5 = patch('%s.AlmConnector.sde_update_task_status' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_sde_update_task_status).start()
        self.mock_6 = patch('%s.AlmConnector.sde_get_task_content' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_sde_get_task_content).start()
        self.mock_7 = patch('%s.AlmConnector._add_note' % PATH_TO_ALM_CONNECTOR, MOCK_SDE.mock_add_note).start()

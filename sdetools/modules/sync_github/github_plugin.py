...
from sdetools.sdelib.restclient import CONF_OPTS
...
class GitHubAPI(RESTBase):
    ...
    def __init__(self, config):
        conf_opts = CONF_OPTS[:]
        conf_opts.append(('alm_api_token', 'API Token for SDE', ''))
        super(GitHubAPI, self).__init__('alm', 'GitHub', config, '', conf_opts)

    def post_conf_init(self):           
        if self._get_conf('api_token'):
            self.auth_mode = 'api_token'
            self.API_TOKEN_HEADER = 'Authorization'
            self.config['alm_api_token'] = 'token %s' % self._get_conf('api_token')
...
class GitHubConnector(AlmConnector):
    ...
    def __init__(self, config, alm_plugin):
        ...
        config.add_custom_option(self.ALM_PRIORITY_MAP, 'Customized map from priority in SDE to GITHUB '
                                 '(JSON encoded dictionary of strings)', default='{"1-6":"Low", "7-9":"Medium", "10":"High"}')

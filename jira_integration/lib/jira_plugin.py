...
class JIRATask(AlmTask):
     ...
     def __init__(self, task_id, alm_id, priority,
                  status, resolution, timestamp, done_statuses):
...
class JIRAConnector(AlmConnector):
    ...
    def __init__(self, sde_plugin, alm_plugin):
        ...
        if (not(self.sde_plugin.config['jira_done_statuses']) or
            ...
            raise AlmException('Missing jira_done_statuses in ' +
                               'configuration')
        else:
    ...
    def alm_connect(self):
        ...
        try:
            ...
            if not(self.jira_issue_type_id):
                raise AlmException('Issue type ' +
                               '%s not available' % self.sde_plugin.config[
                                   'jira_issue_type'])
        ...
        try:
            if ((status=='DONE') or (status=='NA')):
                if not(self.close_transition_id):
                    transitions = self.alm_plugin._call_api('issue' +
                                                      '/%s/transitions' %
                                                      task.get_alm_id())
                    for transition in transitions['transitions']:
                        if (transition['name'] == self.sde_plugin.config[
                                                 'jira_close_transition']):
                            self.close_transition_id = transition['id']
                            break
                    if not(self.close_transition_id):
                        raise AlmException('Unable to find transition ' + 
                                           '%s' % self.sde_plugin.config[
                                                 'jira_close_transition'])

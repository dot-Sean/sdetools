...
class TestALMCase(unittest.TestCase):
     ...
     def __get_changed_status(self,status):
     ...
     def test_sde_update_task_status(self):
          ...
          for task in self.sde_tasks:
               ...
               logging.info("Testing to change status from" +
                            " %s to %s for %s" % (
                                 task['status'],
                                 self.__get_changed_status(task['status']),
                                 task['id']))
                            
               self.tac.sde_update_task_status(
                    task, self.__get_changed_status(task['status']))
     ...
     def test_scoping(self):
          ...
          self.tac.sde_plugin.config['alm_phases'].append(self.
                                                       sde_tasks[0]['phase'])
